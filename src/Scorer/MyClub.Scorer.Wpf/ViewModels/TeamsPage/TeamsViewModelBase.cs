...
{
    internal abstract class TeamsViewModelBase<T> : SelectionListViewModel<T> where T : IEditableTeamViewModel
    {
        ...
        protected TeamsViewModelBase(bool canImport, IListParametersProvider? parametersProvider = null)
        ...
        protected TeamsViewModelBase(ISourceProvider<T> teamsProvider, bool canImport, IListParametersProvider? parametersProvider = null)
        ...
        private async Task SpeedAddAsync(int count)
        {
            ...
            Math.Max(1, count).Iteration(_ => newNames.Add(MyClubResources.Team.Increment(currentNames.Union(newNames).ToList(), format: " #")));
