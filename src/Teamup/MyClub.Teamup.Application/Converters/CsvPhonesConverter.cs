...
{
    public class CsvPhonesConverter : DefaultTypeConverter
    {
        public override object? ConvertFromString(string? text, IReaderRow row, MemberMapData memberMapData)
        {
            ...
            foreach (var contact in contacts)
            {
                ...
                result.Add(new PhoneDto { Label = label, Default = isDefault, Value = value });
            }

            return result;
        }

        public override string? ConvertToString(object? value, IWriterRow row, MemberMapData memberMapData) => value is IEnumerable<PhoneDto> contacts
