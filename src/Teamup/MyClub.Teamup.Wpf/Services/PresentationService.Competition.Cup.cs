...
{
    internal class CupPresentationService(CompetitionService service, IViewModelLocator viewModelLocator) : PresentationServiceBase<CupViewModel, CupEditionViewModel, CompetitionService>(service, viewModelLocator)
    {
        public async Task<Guid?> DuplicateAsync(CupViewModel item)
            => await AddAsync(x =>
            {
        ...
        public int Remove(IEnumerable<CupViewModel> oldItems) => !oldItems.Any() ? 0 : Service.Remove(oldItems.Select(x => x.Id).ToList());
    }
}
